...
export class FiltroPeliculasComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.form.valueChanges
                .subscribe((valores: any) => {
                  ...
                  this.escribirParametrosBuscquedaEnURL();
                });
  }

  buscarPeliculas(valores: any) {
    if(valores.titulo){
      this.peliculas = this.peliculas.filter((pelicula: any) => pelicula.titulo.indexOf(valores.titulo) !== -1);
    }

    if(valores.generoId !== 0){
      this.peliculas = this.peliculas.filter((pelicula: any) => pelicula.generos.indexOf(parseInt(valores.generoId)) !== -1);
    }

    if(valores.proximosEstrenos){
      this.peliculas = this.peliculas.filter((pelicula: any) => pelicula.proximosEstrenos);
    }

    if(valores.enCines){
      this.peliculas = this.peliculas.filter((pelicula: any) => pelicula.enCines);
    }
  }
  ...
  private escribirParametrosBuscquedaEnURL(){
    var queryStrings = [];
